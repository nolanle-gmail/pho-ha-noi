...
(async () => {
  ...
  try {
    ...
    check('acked alert stays on active until done (mine_ack)', act.alerts.some(a => a.id === al1.id && a.mine_ack === true), JSON.stringify(act.alerts.map(a => ({ id: a.id, m: a.mine_ack }))));
    r = await fetch(base + `/api/alerts/${al1.id}/complete`, { method: 'POST', headers: H(alsrv.token), body: '{}' });
    check('recipient marks the alert done', r.status === 200);
    act = await j(await fetch(base + '/api/alerts/active', { headers: H(alsrv.token) }));
    check('completed alert drops off active', !act.alerts.some(a => a.id === al1.id));
    const sentList = await j(await fetch(base + '/api/alerts/sent', { headers: H(mgr.token) }));
    const sent1 = sentList.alerts.find(a => a.id === al1.id) || {};
    check('sender sees the ack + done counts', sent1.ack_count === 1 && sent1.done_count === 1, JSON.stringify(sent1));
    check('single-recipient alert auto-closes on done', !sent1.active, JSON.stringify(sent1));
    const acks = await j(await fetch(base + `/api/alerts/${al1.id}/acks`, { headers: H(mgr.token) }));
    check('sender sees who acknowledged + completed', acks.acks.length === 1 && !!acks.acks[0].name && !!acks.acks[0].completed_at, JSON.stringify(acks.acks));
